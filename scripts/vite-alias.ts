import {resolveUrl, trimPath} from '@mi/utils'
...
export const getPathFromPackageRoot = (roots: RegExp[], path: string): string => {
...
export const createAbsolutePaths = (root: string, paths: string[]) => {
  return paths.map((path) => resolveUrl(root, path))
}
...
export const viteAliasCustomResolver = (options: ViteAliasCustomResolverOptions) => {
  ...
  return (source: string, importer: undefined | string) => {
    ...
    const relativePath = getPathFromPackageRoot(workspaceRegexps, leftPath)
